...
namespace SWars {
    public class Validation
    {
        ...
        public static bool Validate(SWars.TerrainInfo entry)
        {
            return true;
        }

        public static bool Validate(SWars.QuadTextureInfo entry)
        {
            return true;
        }

        public static bool Validate(SWars.TriTextureInfo entry)
        {
            return true;
        }

        public static bool Validate(SWars.Vertex entry)
        {
            return true;
        }

        public static bool Validate(SWars.Tri entry)
        {
            return true;
        }

        public static bool Validate(SWars.Quad entry)
        {
            return true;
        }

        public static bool Validate(SWars.LightInfo entry)
        {
            return true;
        }

        public static bool Validate(SWars.MeshDetails entry)
        {
            return true;
        }

        public static bool Validate(SWars.DataBlockI entry)
        {
            return true;
        }

        public static bool Validate(SWars.DataBlockK entry)
        {
            return true;
        }

        public static bool Validate(SWars.DataBlockM entry)
        {
            return true;
        }

        public static bool Validate(SWars.NPCNavPoint entry)
        {
            return true;
        }

        public static bool Validate(SWars.NPCBlockLine entry)
        {
            return true;
        }

        public static bool Validate(SWars.DataBlockP entry)
        {
            return true;
        }

        public static bool Validate(SWars.DataBlockQ entry)
        {
            return true;
        }

        public static bool Validate(SWars.DataBlockR entry)
        {
            return true;
        }

        public static bool Validate(SWars.SubBlockA entry)
        {
            return true;
        }

        public static bool Validate(SWars.SubBlockB entry)
        {
            return true;
        }
        ...
        public static bool Validate(SWars.EntitySubBlock block, bool isLast)
        {
            if (block.zero1 != 0 ||
                block.four1 != 4 ||
                block.zero2 != 0 ||
        ...
        public static bool Validate(SWars.Map map)
        {
            if (map == null)
            {
                return false;
            }
            if (!Validate(map.header))
            {
                return false;
            }
            foreach (var entry in map.terrainData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.quadTexInfo)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.triTexInfo)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.vertices)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.quads)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.meshes)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.lightInfo)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.lightDetail)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.blockIData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.blockKData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.vehicleNavPoints)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.blockMData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.navPoints)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.blockLines)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.blockPData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.blockQData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.blockRData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            if (!Validate(map.subHeaderPreamble))
            {
                return false;
            }
            foreach (var entry in map.subHeaderA)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            if (!Validate(map.subHeaderB))
            {
                return false;
            }
            foreach (var entry in map.subBlockAData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            foreach (var entry in map.subBlockBData)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            for(int i = 0; i < map.entitySubBlockData.Count; ++i)
            {
                if (!Validate(map.entitySubBlockData[i], i == map.entitySubBlockData.Count-1))
                {
                    return false;
                }
            }
            foreach (var entry in map.entitySubBlockData)
            {

            }
            foreach (var entry in map.dataBlockD)
            {
                if (!Validate(entry))
                {
                    return false;
            ...
            if (!Validate(map.entityHeader))
            {
                return false;
            }
            if (!Validate(map.subHeaderD))
            {
                return false;
            }
            return true;
